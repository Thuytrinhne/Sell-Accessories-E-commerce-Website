...
<div class="container-product-filter">
  ...
  <div class="row filter-widget">
    ...
    <div class="col-4 filter-color">
      <h4>{{$name->name}}</h4>
      <div class="row">
        <!-- anh sửa lại 1 ít chỗ này -->
        @foreach($name->product_configurations as $product_configuration)
        @if ($name->name === 'màu')
        <button class="color-option" style="background-color: {{$product_configuration->variation_value}}; color: {{$product_configuration->variation_value}} " onclick="showProducts('{{$product_configuration->variation_value}}')">
        </button>
        @elseif ($name->name === 'size')
        <button class="color-option" style="background-color: {{$product_configuration->variation_value}}; color: {{$product_configuration->variation_value}} " onclick="showProducts('{{$product_configuration->variation_value}}')">
